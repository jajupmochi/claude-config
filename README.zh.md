...
| [`no-ssh-username-probing`](rules/no-ssh-username-probing/RULE.md) | universal | SSH 前先确认确切用户名,试一次,失败就问——绝不循环猜用户名(会触发 fail2ban 自封 IP)。由 `ssh-guard` 钩子强制 |
...
| **ssh-guard** | Claude | PreToolUse（Bash） | 拦截 SSH 用户名探测——短时间内对同一主机试第 2 个不同 `user@host`——正是触发 fail2ban 自封 IP 的模式（exit 2） |
