...
7. **Fence the final summary top AND bottom with a distinctive marker.** When a turn ends with a substantive summary, set it off so it's instantly grabbable while scrolling (especially on a phone): OPEN with a full-width marker line — e.g. `━━━━━━━━━━ 小结 ━━━━━━━━━━` — and CLOSE with a matching marker line. Both ends, not just the top (a bare top-only `---` is easy to miss). Claude Code interleaves tool calls, intermediate output, and prose, so the summary must visibly stand out above AND below — the eye should land on it without reading. The same applies to any auto-running post-summary output (e.g. a review-gate review): give it its own fenced, formatted block, don't let it sprawl as raw text after the summary.
