...
7. **Set off the final summary with a clear delimiter.** When a turn ends with a substantive summary (more than a one-line reply), put a horizontal rule `---` and a short heading (`## 小结` / `## Summary`, or a descriptive title) right before it, so the user instantly sees where the final answer begins — separate from the tool-call narration scrolling above. (Claude Code interleaves tool calls, intermediate output, and prose; without this marker the summary is hard to locate.)
