...
1. **Get the exact username, port, and key from a source of truth** — deploy config (`deploy.yml` `DEPLOY_USER`, CI secrets doc), the project's runbook, a prior session's memory, or **ask the human**. Do not infer any of them from the hostname or try "likely" values. (Record the host's verified answer — user, SSH port, key type — inline once you have it.)
...
**2026-07-17.** An agent needed to read a production box's database. Not knowing the SSH user, it tried several candidate usernames in succession against the prod host on the SSH port. fail2ban's sshd jail counted the failed auths and banned the shared egress IP on the SSH port. Every other port (the app on :5000, staging on :38xxx) stayed open, so it first looked like a host problem — but a TCP test from a bastion showed the SSH port open from *there*. It was a self-inflicted ban, and it also locked out the human, whose laptop shared that egress IP. The correct username was sitting in the repo's `deploy.yml` the whole time.
