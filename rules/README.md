...
| [`root-cause-before-fix`](root-cause-before-fix/RULE.md) | personal | Before fixing any bug: reproduce + locate (`file:line`), read the failing line's history (`git log -L` / blame), compare with the baseline branch to classify regression vs pre-existing latent fragility + answer "why now?", trace the bad value to its origin, then fix at the correct layer and record the finding. Never patch-first |
| [`fallback-discipline`](fallback-discipline/RULE.md) | personal | Fallback / `pass` / swallowed exception judged by scenario: production allows it but MUST log the trigger + why the primary failed; test/dev is presumed guilty — fix on the spot or raise loudly, never silently hide a bug. Dividing question: "if this fires, does someone find out?" |
| [`commit-discipline`](commit-discipline/RULE.md) | universal | Every commit follows conventional-commit format (`type(scope): description`); no empty or one-word messages. For non-native models, the `commit-msg` hook from `scripts/codex_commit_msg.sh` enforces it |
