...
**Resolution:** every rule has a `scope:` field in its frontmatter (e.g. `universal`, `python-research`, `static-site`, `ml-experiment`). The `setup/init-agent-config` skill asks the user which scopes apply per new project, and only writes those rules in.
